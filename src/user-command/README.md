...
| command | command to be executed as username (will be forwarded as is to a 'bash -c <command>' line) | string | if [ "$USERNAME" = "root" ]; then echo TEST="test" >> /etc/environment; else echo TEST="test" >> "/home/$USERNAME/.bashrc"; fi |
