...
if [ "$BREW_FAST_LEVEL" -eq 0 ]; then
  ...
  while ! (
    ...
    HOMEBREW_ACCEPT_EULA=Y brew install --include-test --force ffmpeg libsndfile libsoundio openmpi pyenv pyenv-virtualenv pipx virtualenv boost opencv openvino
