...
if [ "$BREW_FAST_LEVEL" -eq 0 ]; then
  ...
  while ! (
    HOMEBREW_ACCEPT_EULA=Y brew install --include-test --force sevenzip p7zip awk ca-certificates bash zsh oh-my-posh file-formula gnu-sed coreutils grep curl wget bzip2 swig less lesspipe readline
