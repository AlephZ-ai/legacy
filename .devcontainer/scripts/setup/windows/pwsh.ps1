...
function Install-Modules {
  param($ShellCommand)
  Start-Process -FilePath $ShellCommand -ArgumentList '-Command if (!(Test-Path $PROFILE)) { New-Item -ItemType File -Path $PROFILE -Force | Out-Null; }; Install-Module PowerShellGet -ErrorAction Stop -Force -SkipPublisherCheck -AllowClobber; Install-Module PowerShellGet -ErrorAction Stop -Force -SkipPublisherCheck -AllowClobber -AllowPrerelease; Register-PSRepository -Default; Set-Alias -Name awk -Value gawk;' -Wait -NoNewWindow
  foreach ($module in $modules) {
    Start-Process -FilePath $ShellCommand -ArgumentList "-Command Install-Module $module -Force -SkipPublisherCheck -AllowClobber;" -Wait -NoNewWindow
  }
}
