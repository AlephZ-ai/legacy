...
find "$DEVCONTAINER_FEATURES_PROJECT_ROOT" -type f -iname "*.ps1" -exec git update-index --add --chmod=+x "{}" \;
find "$DEVCONTAINER_FEATURES_PROJECT_ROOT" -type f -iname "*.ps1" -exec chmod +x "{}" \;
